...
def combined_memory_gaussians(state: PatientState):
    ...
    count = sum(bank.count for bank in banks)
    reference = banks[0]
    return gaussian_batch_from_raw(RawGaussianParameters(
        ...
        raw_log_support_amplitude=torch.cat([bank.log_support_amplitude for bank in banks]),
        appearance=torch.cat([bank.appearance for bank in banks]),
        appearance_valid=torch.cat([bank.appearance_valid for bank in banks]),
        patient_state_index=torch.zeros(count, dtype=torch.int64, device=reference.centers_ras_mm.device),
